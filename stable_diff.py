...
def validate_job_params(job_dict, job_index):
    """[MODIFIKASI #12] Validasi parameter job dasar."""
...
def parse_batch_json_input(raw_text):
    """[MODIFIKASI #5 & #13] Parser untuk Mode Batch menggunakan JSON standar yang lebih aman."""
    job_list = []
    if not raw_text.strip():
        return []
    try:
        # Mengharapkan format JSON array: [ {job1}, {job2} ]
        ...
        print(f"   > ❌ ERROR saat mem-parse JSON: {e}. Pastikan formatnya benar.")
        return []
    except Exception as e:
        print(f"   > ❌ ERROR tidak terduga saat parsing: {e}")
        return []
...
def generate_stable_diffusion_image(
    ...
    # [MODIFIKASI #1] Penanganan Resolusi Otomatis untuk v1-5
    if "v1-5" in model_id:
    ...
    # [MODIFIKASI #11] Peringatan untuk resolusi tinggi
    if width * height > 1024 * 1024: # Di atas 1 Megapixel
    ...
    # [MODIFIKASI #2] Penanganan Seed yang lebih aman
    if seed is None:
